...
class CoNet_model(torch.nn.Module):
    ...
    def forward(self, user_indices, item_indices_s,item_indices_t, item_vec_s, item_vec_t):
        ...
        '''
        # 将物品的词向量也拼接起来
        vector_s = torch.cat([user_embedding, item_embedding_s, item_vec_s], dim=-1)  # the concat latent vector
        vector_t = torch.cat([user_embedding, item_embedding_t, item_vec_t], dim=-1)
